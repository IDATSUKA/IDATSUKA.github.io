...
- **`length` は 24fps のフレーム数**で、内部で `17k+5` のグリッドに切り上げられます。
  `124` が約 5 秒。上限は **345 フレーム（14.375 秒）** です。グリッド上の次の値 `362` は
  モデルの上限 360 を超えるため拒否されます（実際に「must be between 48 and 360, got 362」
  というエラーを確認済み）。「15 秒」と言われますが、厳密には 14.375 秒が最大です。
