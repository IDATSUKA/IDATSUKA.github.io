...
Windows には 2 つの道があります。

| | 生成する場所 | GPU | ダウンロード | 費用 |
|---|---|---|---|---|
| **ローカル版**（既定） | 手元の GPU | **24GB 以上・Ampere 世代以降が必須** | 40〜120GB | 無料 |
| **API 版**（`-ApiOnly`） | MiniMax のサーバ | **不要** | 約 10GB | 1 本ごと（768P で約 $0.13/秒） |

GPU が条件を満たさない場合は `-ApiOnly` を使ってください。同じ ComfyUI が入り、
同じ MiniMax H3 が使えます。違うのは計算がどこで走るかだけです。
...
へ切り上げられます。上限は **345 フレーム（14.375 秒）** で、グリッド上の次の値 362 は
モデルの上限 360 フレームを超えるため拒否されます。「最長 15 秒」と紹介されますが、
実際に指定できる最大は 14.375 秒です。
